	...
	<div class="page_root" id="root_demo">
		...
		<div class="warp">
		    <div class="container">
				...
				<!-- 推荐座位信息模块(开始) -->
		<div class="warp" v-if="$check_action('/seat_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/seat_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_seat_information = !show_list_seat_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_seat_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 座位信息推荐列表组件 -->
		                <list_seat_information v-if="show_list_seat_information" :list="list_seat_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_seat_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_seat_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_seat_information" :key="i" @click="to_details('seat_information',o,'seat_information_id')">
		                            <td v-for="(oj,n) in fields_seat_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
						show_list_seat_information: true,
				list_seat_information: [],
				vm_arr_seat_information: [
					"seat_information_id",
							"seat_number",
							"number_of_seats",
							"seat_status",
		...
		methods: {
			...
			toToggle(){
				this.isAdmin = !this.isAdmin;
					...
					// 获取座位信息列表
			get_seat_information() {
				let url = "~/api/seat_information/get_list?";
										this.$get(url, {
									"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_seat_information = json.result.list;
					}
				})
			},
		...
		computed: {
				    list_table_order_evaluation() {
		        var list = this.list_seat_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_seat_information.map((o) => {
		                // 第二个中括号是对象的属性
