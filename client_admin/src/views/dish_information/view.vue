                ...
                <el-form-item label="分类" prop="cart_type">
                    <el-select v-model="form.cart_type">
                        <el-option v-for="(o ,i) in list_cart_goods_type" :value="o.name" :key="i"
							...
							<el-col v-if="user_group === '管理员' || $check_field('get','notice') || $check_field('add','notice') || $check_field('set','notice')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="须知" prop="notice">
												<el-input id="notice" v-model="form['notice']" placeholder="请输入须知"
							  v-if="user_group === '管理员' || (form['dish_information_id'] && $check_field('set','notice')) || (!form['dish_information_id'] && $check_field('add','notice'))" :disabled="disabledObj['notice_isDisabled']"></el-input>
	...
	export default {
		...
		methods: {
            ...
            submit_goods(source_id,add_flag){
				let customize_field = []
							customize_field.push({"field_name":"须知","field_value":this.form.notice});
			                this.goods_form = {
                    goods_id: 0,
